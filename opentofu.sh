# include guard
if [[ -n "$_OPENTOFU_SH_INCLUDED" ]]; then
  return
fi
_OPENTOFU_SH_INCLUDED=1
...
function opentofu_backend_create() {

  log_title "OpenTofu backend create"
  ...
  log_info "OpenTofu backend created successfully"
}

# wraps opentofu commands
function opentofu_run() {

  log_title "OpenTofu run"
  ...
  cd - > /dev/null 2>&1
  log_info "OpenTofu run completed successfully"
  (echo >&2)
}
